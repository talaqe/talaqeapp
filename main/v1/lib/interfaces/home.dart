...
import 'home_navigator_screens/my_home_doctors_screen.dart';
...
int accountid=0;
  ...
  void _setCurrentTab() {
    ...
    switch (_currentPageIndex) {
      case homeScreenIndex:
        _nestedScreenWidget = new HomeScreen();
        break;
      case notificationsScreenIndex:
        _nestedScreenWidget = NotificationScreen();
        break;
      case searchScreenIndex:
        _nestedScreenWidget =  ProductScreen();
        break;
      case appointmentsScreenIndex:
        _nestedScreenWidget = MyProductScreen();
        break;
      case settingsScreenIndex:
        _nestedScreenWidget = MyStatusDoctorsScreen();
  ...
  Future<void> getdata() async {
    ...
    if(login) {
      ...
      var responsenotify = await CallApi().getallData("getnumbernotification/" + dentist.city.toString()+"/" +dentist.university.toString()+"/" +dentist.id.toString());

      var bodynotify = json.decode(utf8.decode(responsenotify.bodyBytes));
      int count=bodynotify??0;
      print("countcount"+count.toString());
